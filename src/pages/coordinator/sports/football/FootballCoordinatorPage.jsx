...
import { FootballEventsTab } from '../../../../components/coordinator/tabs/FootballEventsTab';
import { FootballMatchScheduleTab } from '../../../../components/coordinator/tabs/FootballMatchScheduleTab';
import { FootballLiveMatchControlTab } from '../../../../components/coordinator/tabs/FootballLiveMatchControlTab';
...
export const FootballCoordinatorPage = () => {
  const navigate = useNavigate();
  const { addToast } = useToast();

  const [user, setUser] = useState(() => {
    const current = coordinatorApi.getCurrentUser();
    if (current && current.assignedSport === 'football') return current;
    const preset = coordinatorApi.getPresetAccount ? coordinatorApi.getPresetAccount('football') : null;
    return preset || {
  ...
  useEffect(() => {
    ...
    const fetchAllData = async (showLoading = false) => {
      try {
        ...
        if (showLoading) addToast('Error loading football operations console', 'error');
      } finally {
      ...
      <main className="flex-1 p-3 sm:p-4 max-w-[1600px] w-full mx-auto space-y-3">
        ...
        {loading ? (
          <div className="py-20 text-center space-y-2">
            ...
            <p className="text-xs font-mono text-slate-400">Loading Football operations console...</p>
          </div>
        ) : (
          <>
            {activeTab === 'profile' && (
            ...
            {activeTab === 'events' && (
              <FootballEventsTab user={user} />
            )}

            {activeTab === 'schedule' && (
              <FootballMatchScheduleTab
                matches={matches}
                user={user}
