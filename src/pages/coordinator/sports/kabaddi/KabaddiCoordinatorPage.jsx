...
import { KabaddiEventsTab } from '../../../../components/coordinator/tabs/KabaddiEventsTab';
...
export const KabaddiCoordinatorPage = () => {
  ...
  const [user, setUser] = useState(() => {
    ...
    if (current && current.assignedSport === 'kabaddi') return current;
    const preset = coordinatorApi.getPresetAccount ? coordinatorApi.getPresetAccount('kabaddi') : null;
    return preset || {
      username: 'coord_kabaddi',
  ...
  useEffect(() => {
    ...
    const fetchAllData = async (showLoading = false) => {
      try {
        ...
        if (showLoading) addToast('Error loading Kabaddi operations console', 'error');
      } finally {
      ...
      <main className="flex-1 p-3 sm:p-4 max-w-[1600px] w-full mx-auto space-y-3">
        ...
        {loading ? (
          <div className="py-20 text-center space-y-2">
            ...
            <p className="text-xs font-mono text-slate-400">Loading Kabaddi operations console...</p>
          </div>
        ) : (
          <>
            {activeTab === 'profile' && (
            ...
            {activeTab === 'events' && (
              <KabaddiEventsTab user={user} />
            )}

            {activeTab === 'schedule' && (
              <KabaddiMatchScheduleTab
                matches={matches}
                user={user}
