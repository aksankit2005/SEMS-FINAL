...
import { GullyCricketEventsTab } from '../../../../components/coordinator/tabs/GullyCricketEventsTab';
import { GullyCricketMatchScheduleTab } from '../../../../components/coordinator/tabs/GullyCricketMatchScheduleTab';
import { GullyCricketLiveMatchControlTab } from '../../../../components/coordinator/tabs/GullyCricketLiveMatchControlTab';
import { GullyCricketResultManagementTab } from '../../../../components/coordinator/tabs/GullyCricketResultManagementTab';
import { GullyCricketTotalParticipationTab } from '../../../../components/coordinator/tabs/GullyCricketTotalParticipationTab';
...
export const GullyCricketCoordinatorPage = () => {
  ...
  const [user, setUser] = useState(() => {
    ...
    if (current && current.assignedSport === 'gully-cricket') return current;
    const preset = coordinatorApi.getPresetAccount ? coordinatorApi.getPresetAccount('gully-cricket') : null;
    return preset || {
      username: 'coord_gully_cricket',
      assignedSport: 'gully-cricket',
      sportName: 'Gully Cricket',
      coordinatorName: 'Chiku Bhai',
      email: 'gullycricket.coord@sems.edu'
    };
  });
      ...
      <main className="flex-1 p-3 sm:p-4 max-w-[1600px] w-full mx-auto space-y-3">
        ...
        {loading ? (
          <div className="py-20 text-center space-y-2">
            ...
            <p className="text-xs font-mono text-slate-400">Loading Gully Cricket operations console...</p>
          </div>
        ) : (
          <>
            {activeTab === 'profile' && (
            ...
            {activeTab === 'events' && (
              <GullyCricketEventsTab user={user} />
            )}

            {activeTab === 'schedule' && (
              <GullyCricketMatchScheduleTab
                matches={matches}
                user={user}
            ...
            {activeTab === 'live-control' && (
              <GullyCricketLiveMatchControlTab
                matches={matches}
                user={user}
            ...
            {activeTab === 'results' && (
              <GullyCricketResultManagementTab
                user={user}
                globalSearch={globalSearch}
              />
            )}
            ...
            {activeTab === 'participants' && (
              <GullyCricketTotalParticipationTab
                registrations={registrations}
                user={user}
