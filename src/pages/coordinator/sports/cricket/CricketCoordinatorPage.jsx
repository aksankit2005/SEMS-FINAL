...
import { CricketEventsTab } from '../../../../components/coordinator/tabs/CricketEventsTab';
import { CricketMatchScheduleTab } from '../../../../components/coordinator/tabs/CricketMatchScheduleTab';
import { CricketLiveMatchControlTab } from '../../../../components/coordinator/tabs/CricketLiveMatchControlTab';
...
export const CricketCoordinatorPage = () => {
  const navigate = useNavigate();
  const { addToast } = useToast();

  const [user, setUser] = useState(() => {
    const current = coordinatorApi.getCurrentUser();
    if (current && current.assignedSport === 'cricket') return current;
    const preset = coordinatorApi.getPresetAccount ? coordinatorApi.getPresetAccount('cricket') : null;
    return preset || {
      ...
      <main className="flex-1 p-3 sm:p-4 max-w-[1600px] w-full mx-auto space-y-3">
        ...
        {loading ? (
          <div className="py-20 text-center space-y-2">
            ...
            <p className="text-xs font-mono text-slate-400">Loading Cricket operations console...</p>
          </div>
        ) : (
          <>
            {activeTab === 'profile' && (
            ...
            {activeTab === 'events' && (
              <CricketEventsTab user={user} />
            )}

            {activeTab === 'schedule' && (
              <CricketMatchScheduleTab
                matches={matches}
                user={user}
