...
import { FootballRosterSetupModal } from '../modal/FootballRosterSetupModal';
import { FootballLiveScoreControllerModal } from '../modal/FootballLiveScoreControllerModal';

export const FootballLiveMatchControlTab = ({ matches, user, onUpdateMatchScore }) => {
  const { addToast } = useToast();
  const assignedSport = 'football';

  // Strictly 2 Football Grounds
  const venueCards = ['Ground 1', 'Ground 2'];
      ...
      if (matchItem && matchItem.id && matchItem.status === 'running') {
        try {
          await coordinatorApi.updateMatchScoring(matchItem.id, {
            ...matchItem,
            status: 'running',
            venue: matchItem.tableNumber || 'Ground 1',
          });
        } catch (err) {}
      }
    });
  ...
  const executePromoteGoLive = async (matchItem, targetVenue, r1, r2) => {
    const liveObj = {
      ...matchItem,
      id: matchItem.id || `M-FTB-${Math.floor(100000 + Math.random() * 900000)}`,
      sportId: 'football',
      sportName: 'Football',
      format: 'Team Match (5v5)',
      quarter: matchItem.quarter || '1st Half',
    ...
    addToast(`⚽ Football Match "${matchItem.team1} vs ${matchItem.team2}" is LIVE on ${targetVenue}!`, 'success');
  };
  ...
  const handleCompleteMatch = async (venue) => {
    ...
    addToast(`Football Match on ${venue} completed! Result PDF downloaded.`, 'success');
  };
  ...
  return (
    <div className="space-y-8 text-slate-900 dark:text-slate-200 animate-fade-in font-sans">
                ...
                <div className="space-y-4">
                  ...
                  <div className="text-center space-y-1">
                    <div className="text-xs font-mono font-bold text-emerald-500 uppercase">
                      {activeLive.quarter || '1st Half'} · Goals Continue
                    </div>
                    <h2 className="text-xl font-black text-slate-900 dark:text-white tracking-tight">
                      ...
                      <span className="text-teal-500 font-mono font-black">{activeLive.score2 || 0}</span> {activeLive.team2}
                    </h2>
                    <p className="text-[11px] font-mono text-slate-500 dark:text-slate-400">
                      #{activeLive.id} · {activeLive.eventTitle || 'Football Championship'}
                    </p>
                  </div>
                  ...
                  >
                    <span>🎮 Open Football Score & Sub Controller</span>
                  </button>
          ...
          <div className="space-y-4 pt-4 border-t border-slate-200 dark:border-slate-800">
            <div className="flex items-center justify-between">
              <h3 className="text-base font-black text-slate-900 dark:text-white uppercase tracking-wider">
                Promote Scheduled Football Match to Live
              </h3>
              <span className="text-xs font-mono text-slate-400">Target Grounds: Ground 1 or Ground 2</span>
            </div>
            ...
            {upcomingMatchesToPromote.length === 0 ? (
              <div className="p-8 text-center bg-white dark:bg-[#111827] rounded-3xl border border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 text-xs font-medium shadow-soft dark:shadow-md">
                No upcoming scheduled football matches available to promote. All completed matches have been moved to Results.
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {upcomingMatchesToPromote.map((m) => {
                  const targetGround = m.tableNumber && m.tableNumber.includes('Ground')
                    ? m.tableNumber
                    : 'Ground 1';
                  ...
                  return (
                    ...
                    >
                      <div className="space-y-1">
                        <span className="px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-emerald-50 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/30 uppercase">
                          TEAM MATCH (5v5)
                        </span>
      ...
      {activeControllerVenue && (
        <FootballLiveScoreControllerModal
