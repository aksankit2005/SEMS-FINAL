...
export const KabaddiMatchScheduleTab = ({ matches, user, onUpdateMatches, globalSearch }) => {
  const { addToast } = useToast();
  const { confirmDelete } = useConfirm();

  const assignedSport = 'kabaddi';
  const sportName = 'Kabaddi';
  ...
  const [form, setForm] = useState({
    ...
    category: 'Open',
    eventTitle: 'Kabaddi Championship 2026',
    team1: '',
    team2: '',
    team1Id: '',
    team2Id: '',
    tableNumber: 'Kabaddi Mat Arena 1',
    ...
    const fetchEvents = async () => {
      try {
        ...
        const filtered = (list || []).filter(
          (e) => (e.sportId || e.sportName || '').toLowerCase().includes('kabaddi') || (e.title || '').toLowerCase().includes('kabaddi')
        );
  ...
  useEffect(() => {
    ...
    const loadCompetitors = async () => {
      ...
      try {
        ...
        console.warn('Error loading eligible kabaddi teams:', e);
      } finally {
  ...
  const handleClearAll = async () => {
    const isConfirmed = await confirmDelete({
      title: 'Clear All Kabaddi Matches',
      message: 'Are you sure you want to clear all scheduled Kabaddi matches? This action cannot be undone.'
    });
    if (isConfirmed) {
      await coordinatorApi.clearAllSchedules();
      onUpdateMatches([]);
      addToast('All Kabaddi match schedules cleared', 'warning');
    }
  };
  ...
  const handleAddSlot = async (e) => {
    ...
    if (editingId) {
      ...
      addToast('Kabaddi match fixture scheduled successfully!', 'success');
    }
  ...
  const handleDeleteSlot = async (id) => {
    const isConfirmed = await confirmDelete({
      ...
      message: 'Are you sure you want to delete this scheduled Kabaddi match fixture from the database?'
    });
      ...
      <div className="lg:col-span-4 space-y-4">
        ...
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-wider">
            {editingId ? 'Edit Kabaddi Fixture' : 'Add Kabaddi Fixture'}
          </h3>
          <button
        ...
        {!selectedEvent ? (
          <div className="p-5 rounded-3xl bg-amber-500/10 border border-amber-500/30 text-amber-600 dark:text-amber-400 space-y-1.5">
            <h4 className="text-xs font-black uppercase tracking-wider">⚠️ No Active Kabaddi Event</h4>
            <p className="text-xs text-slate-600 dark:text-slate-400">
              Create and publish a Kabaddi event in the Events tab to enable match scheduling.
            </p>
          </div>
        ) : !isRegClosed ? (
          ...
          <form onSubmit={handleAddSlot} className="space-y-4">
            ...
            {isRegClosed && eligibleCompetitors.teams.length === 0 && (
              <p className="text-[11px] font-bold text-rose-500 dark:text-rose-400">
                ⚠️ No verified registrations found for this Kabaddi event.
              </p>
            )}

            {/* Venue Arena Dropdown */}
            <div>
              <label className="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-1 uppercase">
              ...
              <p className="text-[10px] text-slate-400 mt-1">Select between Arena 1 and Arena 2</p>
            </div>
              ...
              <button
                ...
                onClick={() => {
                  ...
                  setForm({
                    format: 'Pro Style (7 Players)',
                    category: 'Open',
                    eventTitle: createdEvents[0]?.title || 'Kabaddi Championship 2026',
                    team1: '',
                    team2: '',
                    tableNumber: 'Kabaddi Mat Arena 1',
                    date: new Date().toISOString().split('T')[0],
      ...
      <div className="lg:col-span-8 space-y-3">
        ...
        {filteredMatches.length === 0 ? (
          <div className="p-12 text-center bg-white dark:bg-[#111827] rounded-3xl border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 shadow-soft dark:shadow-md">
            No scheduled Kabaddi matches found. Add your first match fixture using the form on the left.
          </div>
        ) : (
