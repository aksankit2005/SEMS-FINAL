...
export const GullyCricketEventsTab = ({ user }) => {
  ...
  // Form State specifically for Gully Cricket (Squad 5-8 players, 6-Overs Fast Box)
  const [formData, setFormData] = useState({
    ...
    status: 'Published',
    rules: [
      '1. Squad Composition: 5 to 8 players per squad (6 players on pitch per innings).',
      ...
      '4. Boundary Rules: Hitting out of designated box/street zone is declared OUT.',
      ...
      '8. Equipment: Official tennis balls provided. Rubberized gully bats allowed.',
      '9. Conduct: Fair play is mandatory. Umpire / Referee decision is final on the street pitch.',
      '10. Valid College Student ID Card mandatory for all registered players.'
    ],
  ...
  const fetchEvents = async () => {
    try {
      ...
      const gullyRegs = (allRegs || []).filter((d) => 
        (d.sportId === 'gully-cricket') || 
        (!d.sport || d.sport.toLowerCase().includes('gully') || d.eventTitle?.toLowerCase().includes('gully'))
      );
      const mapped = (list || []).map((ev) => {
        const matching = gullyRegs.filter((r) => 
          r.eventId === ev.id || 
          r.eventTitle === ev.title || 
          (list.length === 1 && gullyRegs.length > 0)
        );
        return {
          ...ev,
  ...
  const handleOpenEdit = (eventObj) => {
    ...
    setFormData({
      ...
      contactName: eventObj.contactInfo?.name || eventObj.contactName || user?.coordinatorName || 'Chiku Bhai',
      contactEmail: eventObj.contactInfo?.email || eventObj.contactEmail || user?.email || 'gullycricket.coord@sems.edu',
  ...
  const handleOpenCreate = () => {
    ...
    setFormData({
      title: 'Inter-College Gully & Box Cricket Championship 2026',
      sportName: 'Gully Cricket',
      coverImage: 'https://images.unsplash.com/photo-1540747913346-19e32dc3e97e?auto=format&fit=crop&w=800&q=80',
      description: 'Official inter-college 6-Overs Fast Box Gully Cricket tournament. Register your squad (5 - 8 players) for street pitch action!',
      regStartDate: new Date().toISOString().split('T')[0],
      regEndDate: '2026-09-15',
      tournStartDate: '2026-09-16',
      tournEndDate: '2026-09-18',
      ...
      rules: [
        ...
        '6. No LBW: Traditional LBW rules do not apply in Gully Cricket.',
        '7. Toss & Roster: Team captains must submit verified roster 15 minutes before match start.',
        '8. Equipment: Tennis balls provided. Rubberized gully bats allowed.',
        '9. Conduct: Fair play mandatory. Umpire decision is final on the street pitch.',
        '10. Valid College Student ID Card mandatory for all squad members.'
      ],
      requiredDocuments: ['College Student ID Card', 'Aadhaar Card / Govt ID', 'Team Roster Approval Form'],
      contactName: user?.coordinatorName || 'Chiku Bhai',
      contactEmail: user?.email || 'gullycricket.coord@sems.edu',
    ...
    setRulesInput(
      ...
      '6. No LBW: Traditional LBW rules do not apply in Gully Cricket.\n' +
      '7. Valid College Student ID Card mandatory.'
    );
  ...
  const handleViewParticipants = async (eventObj) => {
    ...
    try {
      ...
      const matching = gullyRegs.filter((r) => 
        ...
        (events.length === 1 && gullyRegs.length > 0)
      );
      setParticipants(matching.length > 0 ? matching : gullyRegs);
    } catch (e) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-[#0B1120] p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-soft dark:shadow-md">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-bold text-slate-500 dark:text-slate-400 font-mono">• 6-Overs Box Cricket Configurator</span>
          </div>
          <h2 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white tracking-tight">
            Gully Cricket Tournament Event Management
          </h2>
          <p className="text-xs text-slate-600 dark:text-slate-400 mt-1">
            Configure official 6-Overs Fast Box Gully Cricket team events with flat registration fees and squad controls (5 to 8 players).
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="py-16 text-center space-y-3 bg-white dark:bg-[#0B1120] rounded-3xl border border-slate-200 dark:border-slate-800 p-8 shadow-soft dark:shadow-md">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 max-w-md mx-auto">
              Click the "Create Gully Cricket Event" button above to publish your first 6-Overs Fast Box registration event.
            </p>
            <button
              onClick={handleOpenCreate}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {events.map((event) => {
              ...
              const maxP = event.maxPlayers !== undefined ? event.maxPlayers : (event.maxMembers !== undefined ? event.maxMembers : 8);
                  ...
                  <div className="p-5 space-y-4 flex-1 flex flex-col justify-between">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-xs text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-900/60 p-3 rounded-2xl border border-slate-200 dark:border-slate-800/80">
                      ...
                      <div>
                        ...
                        <span className="font-bold text-emerald-600 dark:text-emerald-400 text-[11px] truncate block">{event.venue || 'Street Pitch Ground 1'}</span>
                      </div>
                      <div>
            ...
            <form onSubmit={handleSaveEvent} className="space-y-5">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4">
                  ...
                  <div className="flex-1 space-y-2 w-full">
                    <p className="text-[11px] text-slate-600 dark:text-slate-400">
                      Upload a high-resolution Gully Cricket cover banner. Click "Crop & Resize" to trim to standard 16:9 banner format before publishing.
                    </p>
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  placeholder="Enter detailed description of Gully Cricket tournament rules, 6-overs box format, pitch constraints, eligibility..."
              ...
              <div className="p-4 rounded-2xl bg-emerald-500/5 border border-emerald-500/20 space-y-4">
                <div className="flex items-center gap-2 border-b border-emerald-500/20 pb-2">
                  ...
                  <h4 className="text-xs font-black uppercase text-emerald-700 dark:text-emerald-400 tracking-wider">
                    Gully Cricket Team Pricing & Squad Limits (6-Overs Box)
                  </h4>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                  <div className="space-y-1 sm:col-span-2">
                    ...
                    />
                    <p className="text-[10px] text-slate-500 dark:text-slate-400">Flat registration fee collected per participating gully cricket squad.</p>
                  </div>
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  placeholder="e.g. Street Pitch Ground 1"
              ...
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  placeholder="Official 6-Overs box cricket rules apply&#10;Min 5 and Max 8 players required per squad&#10;Direct hit out & one-tip one-hand out applicable"
                ...
                <button
                  onClick={() => {
                    const csvData = participants.map((p) => ({
                      ...
                      SquadCount: p.squadCount || '5-8 Players',
