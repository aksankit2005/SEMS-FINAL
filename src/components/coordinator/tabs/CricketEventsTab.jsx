...
export const CricketEventsTab = ({ user }) => {
  ...
  // Form State specifically for Cricket (Team Fee only, Min 11 & Max 15 players)
  const [formData, setFormData] = useState({
    ...
    minPlayers: 11, // Min players per squad
    maxPlayers: 15, // Max players per squad
    teamSize: '11 - 15 Players',
    registeredCount: 0,
    venue: 'Cricket Stadium Ground 1',
    category: 'Open', // Boys, Girls, Open
    status: 'Published', // Draft, Upcoming, Published, Closed
    rules: [
      '1. Team Composition: Each playing team consists of 11 players on the field (Squad size 11-15 players). Substitutions allowed per ICC/BCCI rules.',
      '2. Match Format: T20 Limited Overs format (20 overs per innings) with a 15-minute innings break.',
      '3. Bowling Restrictions: Maximum 4 overs per bowler. Mandatory Powerplay in overs 1 to 6.',
      '4. Scoring & Extras: Standard ICC scoring for Wide, No-Ball, Leg Bye, Bye, and Penalty runs. Free hit awarded on No-Ball for overstepping.',
      '5. Toss & Playing 11: Team captains must submit verified playing 11 list to match referee 15 minutes before scheduled toss time.',
      '6. Ball & Equipment: Standard white/red leather cricket balls provided by committee. Full cricket protective gear (pads, helmet, gloves) mandatory.',
      '7. Rain & DLS Method: Duckworth-Lewis-Stern (DLS) method applied in case of rain interruptions. Minimum 5 overs required per team for a valid match.',
      '8. Over Rate & Penalty: Teams must complete 20 overs within 80 minutes. Slow over rate incurs fielding penalties.',
      '9. Uniforms & Conduct: Matching team jerseys with visible numbers mandatory. Misconduct, swearing, or dissent leads to warning/disqualification.',
      '10. Guidelines: Report 30 mins early. Valid College ID mandatory. Late arrival may cause walkover.'
    ],
  ...
  const fetchEvents = async () => {
    try {
      ...
      addToast('Error loading cricket events console', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOpenEdit = (eventObj) => {
    ...
    const minP = eventObj.minPlayers !== undefined ? eventObj.minPlayers : 11;
    const maxP = eventObj.maxPlayers !== undefined ? eventObj.maxPlayers : 15;
    ...
    setFormData({
      ...
      venue: eventObj.venue || 'Cricket Stadium Ground 1',
  ...
  const handleOpenCreate = () => {
    ...
    setFormData({
      title: 'Inter-College T20 Cricket Championship 2026',
      sportName: 'Cricket',
      coverImage: 'https://images.unsplash.com/photo-1531415074968-036ba1b575da?auto=format&fit=crop&w=800&q=80',
...
Team Composition: Each playing team consists of 11 players on the field. Substitutions allowed as per tournament rules.
Match Format: T20 Limited Overs format (20 overs per innings) with 15-min innings break.
Bowling Limit: Max 4 overs per bowler. Powerplay in overs 1 to 6.
Scoring: Standard ICC rules apply for runs, boundaries, wide, no-ball, free hit.
Toss & Playing 11: Captains submit playing 11 list to match referee 15 minutes before toss.
Balls & Gear: White/red leather balls provided. Full cricket gear (pads, helmet, gloves) mandatory.
Rain Policy: DLS method applied for rain delays. Min 5 overs per team for valid match.
Sportsmanship & Uniforms: Matching jerseys mandatory. Misconduct leads to warning/disqualification.
Organizer's Decision: Umpire and referee decisions are final and binding.

📌 Additional Guidelines:
- Teams should report 30 minutes before scheduled match.
- Carry a valid college ID card.
- Late arrival may result in a walkover.
- Only registered players allowed to participate.`);
    setDocInput('College Student ID Card\nAadhaar Card / Govt ID\nTeam Playing 11 Roster Form');
    setShowCreateModal(true);
  };
  ...
  const handleCropComplete = (croppedDataUrl) => {
    ...
    addToast('Cricket cover banner cropped and attached successfully!', 'success');
  };
  ...
  const handleSaveEvent = async (e) => {
    ...
    try {
      if (editingEvent) {
        ...
        addToast(`Cricket registration event "${updated.title}" updated!`, 'success');
      } else {
        const created = await coordinatorApi.createEvent(eventPayload);
        setEvents((prev) => [created, ...prev]);
        addToast(`New cricket registration event "${created.title}" published!`, 'success');
      }

      setShowCreateModal(false);
      fetchEvents();
    } catch (err) {
      addToast('Failed to save cricket registration event', 'error');
    }
  };

  const handleDeleteEvent = async (id, title) => {
    if (window.confirm(`Are you sure you want to delete cricket event "${title}"?`)) {
      try {
        await coordinatorApi.deleteEvent(id);
        setEvents((prev) => prev.filter((item) => item.id !== id));
        addToast('Cricket event deleted successfully', 'info');
      } catch (err) {
  ...
  return (
    <div className="space-y-6 animate-fade-in font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-[#0B1120] p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-soft dark:shadow-md">
        <div>
          ...
          <h2 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white tracking-tight">
            Cricket Tournament Event Management
          </h2>
          <p className="text-xs text-slate-600 dark:text-slate-400 mt-1">
            Configure Cricket team events with flat team entry fees and squad size controls (11 to 15 players).
          </p>
        </div>
        ...
        >
          ...
          <span>Create Cricket Event</span>
        </button>
      </div>
      ...
      <div className="space-y-4">
        {loading ? (
          <div className="py-16 text-center space-y-2 bg-white dark:bg-[#0B1120] rounded-3xl border border-slate-200 dark:border-slate-800">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">Loading Cricket registration events...</p>
          </div>
        ) : events.length === 0 ? (
          <div className="py-16 text-center space-y-3 bg-white dark:bg-[#0B1120] rounded-3xl border border-slate-200 dark:border-slate-800 p-8 shadow-soft dark:shadow-md">
            ...
            <h3 className="text-base font-black text-slate-900 dark:text-white">No Cricket Events Created Yet</h3>
              ...
              return (
                ...
                >
                  ...
                  <div className="relative h-44 w-full bg-slate-900 overflow-hidden">
                    <img
                      src={event.coverImage || 'https://images.unsplash.com/photo-1531415074968-036ba1b575da?auto=format&fit=crop&w=800&q=80'}
                      alt={event.title}
                  ...
                  <div className="p-5 space-y-4 flex-1 flex flex-col justify-between">
                    ...
                    <div className="flex items-center justify-between text-xs pt-1">
                      ...
                      <span className="font-mono font-black text-emerald-600 dark:text-emerald-400">{registered} Teams Registered</span>
                    </div>
      ...
      {/* CREATE / EDIT CRICKET EVENT MODAL */}
      {showCreateModal && (
            ...
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
              <div>
                <span className="text-[10px] font-mono font-bold uppercase text-emerald-600 dark:text-emerald-400">
                  Cricket Event Configurator
                </span>
                <h3 className="text-xl font-black text-slate-900 dark:text-white">
                  {editingEvent ? 'Edit Cricket Event' : 'Create New Cricket Event'}
                </h3>
              </div>
              <button
            ...
            <form onSubmit={handleSaveEvent} className="space-y-5">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4">
                  ...
                  <div className="flex-1 space-y-2 w-full">
                    <p className="text-[11px] text-slate-600 dark:text-slate-400">
                      Upload a high-resolution Cricket cover image. Click "Crop & Resize" to trim to standard 16:9 banner format before publishing.
                    </p>
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  placeholder="Enter detailed description of Cricket tournament rules, over format, eligibility..."
              ...
              {/* TEAM FEE ONLY, MIN 11 & MAX 15 PLAYERS */}
              <div className="p-4 rounded-2xl bg-emerald-500/5 border border-emerald-500/20 space-y-4">
                <div className="flex items-center gap-2 border-b border-emerald-500/20 pb-2">
                  ...
                  <h4 className="text-xs font-black uppercase text-emerald-700 dark:text-emerald-400 tracking-wider">
                    Cricket Team Pricing & Squad Limits
                  </h4>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                  ...
                  <div className="space-y-1 sm:col-span-2">
                    ...
                    />
                    <p className="text-[10px] text-slate-500 dark:text-slate-400">Flat registration fee collected per participating cricket team.</p>
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    />
                    <p className="text-[10px] font-mono text-slate-500 dark:text-slate-400">Default: 11</p>
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-xs font-bold text-slate-900 dark:text-white"
                    />
                    <p className="text-[10px] font-mono text-slate-500 dark:text-slate-400">Default: 15</p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  placeholder="e.g. Cricket Stadium Ground 1"
              ...
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  placeholder="Official ICC/BCCI Cricket rules apply&#10;Min 11 and Max 15 players required per squad&#10;Leather ball and full protective kit mandatory"
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-slate-200 dark:border-slate-800">
                ...
                >
                  {editingEvent ? 'Save Cricket Event' : 'Publish Cricket Event'}
                </button>
              </div>
                ...
                <button
                  onClick={() => {
                    const csvData = participants.map((p) => ({
                      ...
                      SquadCount: p.squadCount || '11-15',
                      Phone: p.contactPhone || p.phone,
                      Status: p.status,
                      RegisteredDate: p.registeredDate
                    }));
                    exportToCSV(csvData, `Cricket_Participants_${selectedEventForParticipants.id}`);
                    addToast('Exported Cricket Roster as CSV/Excel', 'success');
                  }}
                  ...
                  {filteredParticipants.length === 0 ? (
                    ...
                    filteredParticipants.map((p) => (
                      <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-slate-900/50 transition">
                        ...
                        <td className="p-3 font-mono">11-15 Players</td>
