...
import { GullyCricketRosterSetupModal } from '../modal/GullyCricketRosterSetupModal';
import { GullyCricketLiveScoreControllerModal } from '../modal/GullyCricketLiveScoreControllerModal';

export const GullyCricketLiveMatchControlTab = ({ matches, user, onUpdateMatchScore, globalSearch }) => {
  const { addToast } = useToast();
  const assignedSport = 'gully-cricket';

  const venueCards = ['Ground 1', 'Ground 2', 'Ground 3', 'Ground 4'];
  ...
  const [activeRosterSetupMatch, setActiveRosterSetupMatch] = useState(null);
  ...
  const executePromoteGoLive = async (matchItem, targetVenue, r1, r2) => {
    const liveObj = {
      ...matchItem,
      id: matchItem.id || `M-GCK-${Math.floor(100000 + Math.random() * 900000)}`,
      sportId: 'gully-cricket',
      sportName: 'Gully Cricket',
      format: 'Team Match',
      score1: matchItem.score1 || '0/0',
      score2: matchItem.score2 || '0/0',
      roster1: r1,
      roster2: r2,
      status: 'running',
      ground: targetVenue,
    ...
    addToast(`🏏 Gully Cricket Match "${matchItem.team1} vs ${matchItem.team2}" is LIVE on ${targetVenue}!`, 'success');
  };
  ...
  const handleSaveStreamUrl = async (venue) => {
    ...
    if (rawUrl && !videoId) {
      addToast('Invalid YouTube Live URL', 'error');
      return;
    }
    ...
    await coordinatorApi.updateMatchScoring(active.id, { liveStreamUrl: rawUrl, youtubeVideoId: videoId });
    addToast(`YouTube Live stream attached to ${venue}`, 'success');
  };
  ...
  const handleToggleStreamLive = async (venue, start) => {
    ...
    setLiveAssignments((prev) => ({ ...prev, [venue]: updated }));
    await coordinatorApi.updateMatchScoring(active.id, { isLiveStreaming: start });
    addToast(`YouTube Live Stream ${start ? 'STARTED 📡' : 'STOPPED ⏹'} on ${venue}`, start ? 'success' : 'warning');
  };
  ...
  const handleCompleteMatch = async (venue) => {
    ...
    const winnerName = active.winner || active.team1;
    const completedObj = {
      ...active,
      winner: winnerName,
      status: 'COMPLETED',
    ...
    if (onUpdateMatchScore) onUpdateMatchScore(active.id, { status: 'COMPLETED' });
  ...
  const handleDemoteMatch = async (venue) => {
    const active = liveAssignments[venue];
    if (active) {
      await coordinatorApi.updateMatchScoring(active.id, { status: 'DEMOTED', isLiveStreaming: false });
    }
    setLiveAssignments((prev) => {
      const copy = { ...prev };
      delete copy[venue];
  ...
  return (
    <div className="space-y-8 text-slate-900 dark:text-slate-200 animate-fade-in font-sans">
                ...
                <div className="space-y-4">
                  ...
                  <div className="text-center space-y-1">
                    <div className="text-xs font-mono font-bold text-orange-500 uppercase">
                      {activeLive.eventTitle || 'Gully Cricket Tournament'}
                    </div>
                    <h2 className="text-xl font-black text-slate-900 dark:text-white tracking-tight">
                      {activeLive.team1} <span className="text-orange-500 font-mono font-black">{activeLive.score1 || '0/0'}</span>
                      <span className="text-slate-400 font-normal text-sm px-2">VS</span>
                      <span className="text-blue-500 font-mono font-black">{activeLive.score2 || '0/0'}</span> {activeLive.team2}
                    </h2>
                    <p className="text-[11px] font-mono text-slate-500 dark:text-slate-400">
                      #{activeLive.id} · Ground: {venueName}
                    </p>
                  </div>
                  ...
                  <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-[#090D16] border border-slate-200 dark:border-slate-800 space-y-2.5">
                    ...
                    <div className="flex items-center justify-between gap-2 pt-1">
                      <div className="flex items-center gap-1.5">
                        {activeLive.youtubeVideoId && (
                          <button
                            onClick={() => window.open(`https://youtube.com/watch?v=${activeLive.youtubeVideoId}`, '_blank')}
                  ...
                  >
                    <span>🎮 Open Gully Cricket Score & Sub Controller</span>
                  </button>
        ...
        return (
          <div className="space-y-4 pt-4 border-t border-slate-200 dark:border-slate-800">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs font-mono text-slate-400">Target Grounds: Ground 1 to Ground 4</span>
            </div>
            ...
            {upcomingMatchesToPromote.length === 0 ? (
              <div className="p-8 text-center bg-white dark:bg-[#111827] rounded-3xl border border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 text-xs font-medium shadow-soft dark:shadow-md">
                No upcoming scheduled gully cricket matches available to promote.
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {upcomingMatchesToPromote.map((m) => {
                  const targetGround = m.ground || m.tableNumber || 'Ground 1';
                    ...
                    >
                      <div className="space-y-1">
                        <span className="px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-orange-50 dark:bg-orange-500/20 text-orange-600 dark:text-orange-400 border border-orange-200 dark:border-orange-500/30 uppercase">
                          TEAM MATCH • {m.overs || 6} OVERS
                        </span>
                        <h4 className="text-sm font-black text-slate-900 dark:text-white">
                          {m.team1} vs {m.team2}
                        </h4>
                        <p className="text-[11px] font-mono text-slate-500 dark:text-slate-400">
                          📍 {targetGround} | Time: {m.time || '04:00 PM'}
                        </p>
                      </div>

                      <button
                        onClick={() => handleInitiateGoLive(m, targetGround)}
      ...
      {activeControllerVenue && (
        <GullyCricketLiveScoreControllerModal
