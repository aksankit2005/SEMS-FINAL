...
export const CricketMatchScheduleTab = ({ matches, user, onUpdateMatches, globalSearch }) => {
    ...
    const fetchEvents = async () => {
      try {
        ...
        const filtered = (list || []).filter(
          (e) => (e.sportId || e.sportName || '').toLowerCase().includes('cricket') && !(e.sportId || e.sportName || '').toLowerCase().includes('gully')
        );
  ...
  useEffect(() => {
    ...
    const loadCompetitors = async () => {
      ...
      try {
        ...
        console.warn('Error loading eligible cricket teams:', e);
      } finally {
  ...
  const handleClearAll = async () => {
    const isConfirmed = await confirmDelete({
      title: 'Clear All Cricket Matches',
      message: 'Are you sure you want to clear all scheduled cricket matches? This action cannot be undone.'
    });
    if (isConfirmed) {
  ...
  const handleDeleteSlot = async (id) => {
    const isConfirmed = await confirmDelete({
      title: 'Delete Cricket Match Fixture',
      message: 'Are you sure you want to delete this scheduled cricket match fixture?'
    });
    if (!isConfirmed) return;
    await coordinatorApi.deleteMatch(id);
    onUpdateMatches(matches.filter((m) => m.id !== id));
    addToast('Cricket fixture deleted from database', 'info');
  };
      ...
      <div className="lg:col-span-4 space-y-4">
        ...
        {!selectedEvent ? (
          <div className="p-5 rounded-3xl bg-amber-500/10 border border-amber-500/30 text-amber-600 dark:text-amber-400 space-y-1.5">
            <h4 className="text-xs font-black uppercase tracking-wider">⚠️ No Active Cricket Event</h4>
            <p className="text-xs text-slate-600 dark:text-slate-400">
              Create and publish a Cricket event in the Events tab to enable match scheduling.
            </p>
          </div>
        ) : !isRegClosed ? (
          ...
          <form onSubmit={handleAddSlot} className="space-y-4">
            ...
            {isRegClosed && eligibleCompetitors.teams.length === 0 && (
              <p className="text-[11px] font-bold text-rose-500 dark:text-rose-400">
                ⚠️ No verified registrations found for this Cricket event.
              </p>
            )}
