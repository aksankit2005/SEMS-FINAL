...
export const BasketballMatchScheduleTab = ({ matches, user, onUpdateMatches, globalSearch }) => {
    ...
    const fetchEvents = async () => {
      try {
        ...
        const filtered = (list || []).filter(
          (e) => (e.sportId || e.sportName || '').toLowerCase().includes('basketball') || (e.title || '').toLowerCase().includes('basketball')
        );
  ...
  useEffect(() => {
    ...
    const loadCompetitors = async () => {
      ...
      try {
        ...
        console.warn('Error loading eligible basketball teams:', e);
      } finally {
  ...
  const handleAddSlot = async (e) => {
    ...
    if (editingId) {
      ...
      addToast('Basketball match fixture updated successfully!', 'success');
      setEditingId(null);
    } else {
      const newSlot = {
  ...
  const handleDeleteSlot = async (id) => {
    const isConfirmed = await confirmDelete({
      ...
      message: 'Are you sure you want to delete this scheduled basketball match fixture from the database?'
    });
      ...
      <div className="lg:col-span-4 space-y-4">
        ...
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-wider">
            {editingId ? 'Edit Basketball Fixture' : 'Add Basketball Fixture'}
          </h3>
        </div>
        ...
        {!selectedEvent ? (
          <div className="p-5 rounded-3xl bg-amber-500/10 border border-amber-500/30 text-amber-600 dark:text-amber-400 space-y-1.5">
            <h4 className="text-xs font-black uppercase tracking-wider">⚠️ No Active Basketball Event</h4>
            <p className="text-xs text-slate-600 dark:text-slate-400">
              Create and publish a Basketball event in the Events tab to enable match scheduling.
            </p>
          </div>
        ) : !isRegClosed ? (
          ...
          <form onSubmit={handleAddSlot} className="space-y-4">
            ...
            {isRegClosed && eligibleCompetitors.teams.length === 0 && (
              <p className="text-[11px] font-bold text-rose-500 dark:text-rose-400">
                ⚠️ No verified registrations found for this Basketball event.
              </p>
            )}

            {/* Basketball Court Dropdown */}
            <div>
              <label className="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-1 uppercase">
              ...
              >
                ...
                <option value="Basketball Court 3">Basketball Court 3</option>
                <option value="Basketball Court 4">Basketball Court 4</option>
              </select>
              ...
              >
                <div className="space-y-1.5">
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 font-mono">
                    📍 {displayVenue} | Date: {m.date || new Date().toISOString().split('T')[0]} | Time: {m.time || '04:00 PM'} | Event: {m.eventTitle || 'Basketball Championship'}
                  </p>
                </div>
