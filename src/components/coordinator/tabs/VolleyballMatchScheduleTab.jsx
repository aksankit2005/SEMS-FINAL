...
export const VolleyballMatchScheduleTab = ({ matches, user, onUpdateMatches, globalSearch }) => {
    ...
    const fetchEvents = async () => {
      try {
        ...
        const filtered = (list || []).filter(
          (e) => (e.sportId || e.sportName || '').toLowerCase().includes('volleyball') || (e.title || '').toLowerCase().includes('volleyball')
        );
  ...
  useEffect(() => {
    ...
    const loadCompetitors = async () => {
      ...
      try {
        ...
        console.warn('Error loading eligible volleyball teams:', e);
      } finally {
  ...
  const handleAddSlot = async (e) => {
    ...
    if (editingId) {
      ...
      addToast('Volleyball match fixture updated successfully!', 'success');
      setEditingId(null);
    } else {
      const newSlot = {
  ...
  const handleDeleteSlot = async (id) => {
    const isConfirmed = await confirmDelete({
      ...
      message: 'Are you sure you want to delete this scheduled volleyball match fixture from the database?'
    });
      ...
      <div className="lg:col-span-4 space-y-4">
        ...
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-wider">
            {editingId ? 'Edit Volleyball Fixture' : 'Add Volleyball Fixture'}
          </h3>
        </div>
        ...
        {!selectedEvent ? (
          <div className="p-5 rounded-3xl bg-amber-500/10 border border-amber-500/30 text-amber-600 dark:text-amber-400 space-y-1.5">
            <h4 className="text-xs font-black uppercase tracking-wider">⚠️ No Active Volleyball Event</h4>
            <p className="text-xs text-slate-600 dark:text-slate-400">
              Create and publish a Volleyball event in the Events tab to enable match scheduling.
            </p>
          </div>
        ) : !isRegClosed ? (
          ...
          <form onSubmit={handleAddSlot} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <select
                ...
                className="w-full px-4 py-2.5 rounded-xl bg-slate-50 dark:bg-[#0B1120] border border-slate-200 dark:border-slate-800 text-xs font-semibold text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-orange-500"
              >
            ...
            {isRegClosed && eligibleCompetitors.teams.length === 0 && (
              <p className="text-[11px] font-bold text-rose-500 dark:text-rose-400">
                ⚠️ No verified registrations found for this Volleyball event.
              </p>
            )}

            {/* Volleyball Court Dropdown */}
            <div>
              <label className="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-1 uppercase">
              ...
              >
                <div className="space-y-1.5">
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 font-mono">
                    📍 {displayVenue} | Date: {m.date || new Date().toISOString().split('T')[0]} | Time: {m.time || '04:00 PM'} | Event: {m.eventTitle || 'Volleyball Championship'}
                  </p>
                </div>
