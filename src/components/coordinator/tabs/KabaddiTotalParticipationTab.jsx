...
const DEFAULT_KABADDI_PARTICIPANTS = [
  {
    id: 'REG-KBD-101',
    timestamp: '16 Jul, 10:00 AM',
    sport: 'Kabaddi',
    eventTitle: 'Kabaddi Mens Championship 2026',
    teamName: 'MPEC Raid Stars',
    collegeName: 'MPEC',
    name: 'Aarav Sharma',
    captainName: 'Aarav Sharma',
    phone: '9876543110',
    email: 'aarav.raidstars@sems.edu'
  },
  {
    id: 'REG-KBD-102',
    timestamp: '16 Jul, 11:30 AM',
    sport: 'Kabaddi',
    eventTitle: 'Kabaddi Mens Championship 2026',
    teamName: 'MIPS Defenders',
    collegeName: 'MIPS',
    name: 'Rohan Gupta',
    captainName: 'Rohan Gupta',
    phone: '9876543111',
    email: 'rohan.defenders@sems.edu'
  },
  {
    id: 'REG-KBD-103',
    timestamp: '16 Jul, 03:15 PM',
    sport: 'Kabaddi',
    eventTitle: 'Womens Kabaddi Tournament',
    teamName: 'MPCP Warriors',
    collegeName: 'MPCP',
    name: 'Ankur Dixit',
    captainName: 'Ankur Dixit',
    phone: '9876543112',
    email: 'ankur.warriors@sems.edu'
  },
  {
    id: 'REG-KBD-104',
    timestamp: '17 Jul, 09:45 AM',
    sport: 'Kabaddi',
    eventTitle: 'Kabaddi Mens Championship 2026',
    ...
    phone: '9876543113',
    email: 'aditya.strikers@sems.edu'
  }
];

export const KabaddiTotalParticipationTab = ({ user, globalSearch = '' }) => {
  const { addToast } = useToast();
  const [search, setSearch] = useState('');
  const [participants, setParticipants] = useState([]);

  const sportId = 'kabaddi';
  ...
  const loadData = async () => {
    try {
      ...
      const kbdData = (data || []).filter((d) => 
        !d.sport ||
        d.sport?.toLowerCase().includes('kabaddi') ||
        d.sportId?.toLowerCase().includes('kabaddi') ||
        d.eventTitle?.toLowerCase().includes('kabaddi')
      );

      if (kbdData && kbdData.length > 0) {
        setParticipants(kbdData);
      } else {
        const saved = localStorage.getItem(participantsKey) || localStorage.getItem('sems_participants_kabaddi');
        if (saved) {
          try {
            const parsed = JSON.parse(saved);
            setParticipants(Array.isArray(parsed) && parsed.length > 0 ? parsed : DEFAULT_KABADDI_PARTICIPANTS);
          } catch (e) {
            setParticipants(DEFAULT_KABADDI_PARTICIPANTS);
          }
        } else {
          setParticipants(DEFAULT_KABADDI_PARTICIPANTS);
        }
  ...
  const handleClearParticipants = async () => {
    if (window.confirm('Clear all Kabaddi participant data from storage?')) {
      setParticipants([]);
      localStorage.removeItem(participantsKey);
      localStorage.removeItem('sems_participants_kabaddi');
      addToast('All Kabaddi participant data cleared', 'warning');
    }
  };
