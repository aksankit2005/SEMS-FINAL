...
export const KabaddiTotalParticipationTab = ({ user, globalSearch = '' }) => {
  ...
  const loadData = async () => {
    try {
      ...
      const kabData = (data || []).filter((d) => 
        !d.sport || d.sport.toLowerCase().includes('kabaddi') || d.eventTitle?.toLowerCase().includes('kabaddi')
      );
      setRegistrations(kabData);
    } catch (e) {
      console.error('Failed to load kabaddi registrations:', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExportExcel = () => {
    ...
    exportToCSV(exportData, `Kabaddi_Official_Roster_${genderFilter !== 'ALL' ? genderFilter + '_' : ''}${new Date().toISOString().split('T')[0]}`);
    addToast(`Kabaddi official roster (${genderFilter !== 'ALL' ? genderFilter : 'All'}) exported to CSV successfully!`, 'success');
  };
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 p-5 rounded-2xl shadow-sm">
        <div>
          <h3 className="text-base font-extrabold text-slate-900 dark:text-white flex items-center gap-2">
            ...
            <span>Kabaddi Total Participants</span>
          </h3>
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={10} className="p-12 text-center text-slate-400 dark:text-slate-500 font-mono text-xs">
                    {loading ? 'Loading participants from database...' : 'No registered Kabaddi student athletes found in database.'}
                  </td>
                </tr>
              ) : (
                filtered.map((p, idx) => (
