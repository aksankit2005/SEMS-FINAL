...
export const KabaddiEventsTab = ({ user }) => {
  ...
  // Form State specifically for Kabaddi
  const [formData, setFormData] = useState({
    title: '',
    sportName: 'Kabaddi',
    coverImage: 'https://images.unsplash.com/photo-1561080695-41c3e009207a?auto=format&fit=crop&w=800&q=80',
    description: '',
    ...
    minPlayers: 7,
    maxPlayers: 12,
    teamSize: '7 - 12 Players',
    registeredCount: 0,
    venue: 'Kabaddi Mat Arena 1',
    category: 'Open',
    status: 'Published',
    rules: [
      '1. Team Composition: 7 active players on the mat (Squad size 7-12 players). Substitutions allowed per Pro Kabaddi rules.',
      '2. Match Duration: 2 halves of 20 minutes each with a 5-minute halftime break.',
      '3. Raiding Rule: Raider must continuously chant "Kabaddi" and complete raid within 30 seconds.',
      '4. Scoring: Touch Point = +1 | Bonus Point = +1 (with 6 or 7 defenders on mat) | Super Tackle = +2 (3 or fewer defenders).',
      '5. All-Out / Lona: Eliminating all 7 players awards +2 Lona/All-Out points to opponent and revives all players.',
      '6. Do-or-Die Raid: 3rd consecutive empty raid results in raider being declared out.',
      '7. Lobby Rule: Lobby area becomes active only after touch/contact is made during a raid.'
    ],
  ...
  const fetchEvents = async () => {
    try {
      ...
      addToast('Error loading Kabaddi events console', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOpenEdit = (eventObj) => {
    ...
    const minP = eventObj.minPlayers !== undefined ? eventObj.minPlayers : 7;
    const maxP = eventObj.maxPlayers !== undefined ? eventObj.maxPlayers : 12;
    
    setFormData({
      title: eventObj.title || '',
      sportName: 'Kabaddi',
      coverImage: eventObj.coverImage || 'https://images.unsplash.com/photo-1561080695-41c3e009207a?auto=format&fit=crop&w=800&q=80',
      ...
      venue: eventObj.venue || 'Kabaddi Mat Arena 1',
      category: eventObj.category || 'Open',
      status: eventObj.status || 'Published',
      rules: eventObj.rules || [
        '1. Team Composition: 7 active players on the mat (Squad 7-12 players).',
        '2. Match Duration: 2 halves of 20 minutes each with a 5-minute halftime break.',
        '3. Raiding Rule: Raider must chant "Kabaddi" and complete raid within 30 seconds.',
        '4. Touch & Tackle Scoring: Touch Point = +1 | Super Tackle = +2 | All Out = +2.'
      ],
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingEvent) {
        ...
        addToast('Kabaddi event updated successfully!', 'success');
      } else {
        await coordinatorApi.createEvent(payload);
        addToast('New Kabaddi event published successfully!', 'success');
      }
      ...
      addToast('Failed to save Kabaddi event', 'error');
    }
  };
  ...
  const handleViewParticipants = async (eventObj) => {
    ...
    try {
      ...
      const eventRegs = allRegs.filter((r) => r.eventId === eventObj.id || resolveSportKey(r) === 'kabaddi');
  ...
  const kabaddiEvents = events.filter((e) => {
    if (!e) return false;
    const key = resolveSportKey(e);
    return key === 'kabaddi' || e.sportId === 'kabaddi' || e.assignedSport === 'kabaddi' || e.sportName?.toLowerCase() === 'kabaddi' || !e.sportName;
  });

  const totalEvents = kabaddiEvents.length;
  const activeEvents = kabaddiEvents.filter((e) => e.status === 'Published').length;
  const upcomingEvents = kabaddiEvents.filter((e) => e.status === 'Upcoming').length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white dark:bg-[#0F172A] p-4 sm:p-6 rounded-2xl border border-slate-200 dark:border-[#1E293B] shadow-sm">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Configure Pro-style Kabaddi tournaments, mat arenas, squad size (7-12 players), and registration fees.
          </p>
        </div>
      ...
      {/* Events List Grid */}
      {loading ? (
        <div className="py-12 text-center text-xs text-slate-400">Loading Kabaddi Events...</div>
      ) : kabaddiEvents.length === 0 ? (
        <div className="p-8 text-center bg-white dark:bg-[#0F172A] rounded-2xl border border-slate-200 dark:border-[#1E293B] space-y-3">
          ...
          <h3 className="text-sm font-bold text-slate-700 dark:text-slate-200">No Kabaddi Events Created Yet</h3>
          <p className="text-xs text-slate-400 max-w-sm mx-auto">
            Click the button above to publish your first Kabaddi event and open registrations for college squads.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {kabaddiEvents.map((evt) => (
            <div 
              key={evt.id} 
            ...
            >
              <div>
                ...
                <div className="relative h-44 overflow-hidden bg-slate-900">
                  <img 
                    src={evt.coverImage || 'https://images.unsplash.com/photo-1561080695-41c3e009207a?auto=format&fit=crop&w=800&q=80'} 
                    alt={evt.title} 
                ...
                <div className="p-4 space-y-3">
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    ...
                    <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-[#090D16] border border-slate-100 dark:border-[#1E293B]">
                      ...
                      <span className="font-bold text-slate-700 dark:text-slate-200">
                        {evt.minPlayers || 7} - {evt.maxPlayers || 12} Players
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1.5 text-xs text-slate-500 dark:text-slate-400">
                    <div className="flex items-center justify-between">
                      <span>Venue:</span>
                      <span className="font-semibold text-slate-700 dark:text-slate-200">{evt.venue || 'Kabaddi Mat Arena 1'}</span>
                    </div>
                    <div className="flex items-center justify-between">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Official Inter-College Kabaddi Tournament hosted under APEX 2026 Pro-Style mat guidelines."
                  className="w-full p-3 rounded-xl bg-slate-50 dark:bg-[#090D16] border border-slate-200 dark:border-[#1E293B] text-xs"
                />
              </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-[#1E293B]">
                <div>
                  ...
                  <p className="text-xs text-slate-400">Registered College Kabaddi Squads</p>
                </div>
