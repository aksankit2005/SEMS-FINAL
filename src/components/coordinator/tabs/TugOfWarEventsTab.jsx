...
export const TugOfWarEventsTab = ({ user }) => {
  ...
  // Form State specifically for Tug of War (Team Fee only, Min 8 & Max 10 players)
  const [formData, setFormData] = useState({
    ...
    status: 'Published',
    rules: [
      'Official Tug of War tournament pulling rules apply.',
      'Team squad must consist of minimum 8 and maximum 10 players.',
      'College Student ID & Pass mandatory for all pullers.',
      'Standard athletic footwear and team uniforms strictly required.'
    ],
  ...
  const handleOpenEdit = (eventObj) => {
    ...
    const minP = eventObj.minPlayers !== undefined ? eventObj.minPlayers : 8;
    ...
    setFormData({
      ...
      venue: eventObj.venue || 'Tug of War Ground 1',
  ...
  const handleOpenCreate = () => {
    ...
    setFormData({
      title: 'Inter-College Tug of War Championship 2026',
      sportName: 'Tug of War',
      coverImage: 'https://images.unsplash.com/photo-1517649763962-0c623066013b?auto=format&fit=crop&w=800&q=80',
  ...
  const handleCropComplete = (croppedDataUrl) => {
    ...
    addToast('Tug of War cover banner cropped and attached successfully!', 'success');
  };
  ...
  const handleDeleteEvent = async (id, title) => {
    if (window.confirm(`Are you sure you want to delete tug of war event "${title}"?`)) {
      try {
  ...
  const handleToggleRegistrationOpen = async (eventObj) => {
    ...
    try {
      const updated = await coordinatorApi.updateEvent(eventObj.id, {
        ...
        status: newRegOpen ? 'Published' : 'Closed'
      });
      setEvents((prev) => prev.map((item) => (item.id === eventObj.id ? { ...item, ...updated, registrationOpen: newRegOpen, status: newRegOpen ? 'Published' : 'Closed' } : item)));
      if (!newRegOpen) {
        addToast(`🔒 Registration closed for "${eventObj.title}". Fixtures can now be scheduled!`, 'success');
      } else {
        addToast(`🔓 Registration reopened for "${eventObj.title}". Teams can now register.`, 'info');
      }
      fetchEvents();
      window.dispatchEvent(new Event('sems_events_updated'));
    } catch (err) {
  ...
  const handleToggleEventStatus = async (eventObj, targetStatus) => {
    const nextStatus = targetStatus || ((eventObj.status === 'Active' || eventObj.status === 'Published') ? 'Closed' : 'Published');
    try {
      const updated = await coordinatorApi.updateEvent(eventObj.id, { 
        status: nextStatus,
        registrationOpen: nextStatus === 'Published' || nextStatus === 'Active'
      });
  ...
  return (
    <div className="space-y-6 animate-fade-in font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-[#0B1120] p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-soft dark:shadow-md">
        <div>
          ...
          <h2 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white tracking-tight">
            Tug of War Tournament Event Management
          </h2>
          <p className="text-xs text-slate-600 dark:text-slate-400 mt-1">
            Configure Tug of War team events with flat team entry fees and squad size controls (8 to 10 players).
          </p>
        </div>
        ...
        >
          ...
          <span>Create Tug of War Event</span>
        </button>
      </div>
      ...
      <div className="space-y-4">
        {loading ? (
          <div className="py-16 text-center space-y-2 bg-white dark:bg-[#0B1120] rounded-3xl border border-slate-200 dark:border-slate-800">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">Loading Tug of War registration events...</p>
          </div>
        ) : events.length === 0 ? (
          <div className="py-16 text-center space-y-3 bg-white dark:bg-[#0B1120] rounded-3xl border border-slate-200 dark:border-slate-800 p-8 shadow-soft dark:shadow-md">
            ...
            <h3 className="text-base font-black text-slate-900 dark:text-white">No Tug of War Events Created Yet</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 max-w-md mx-auto">
              Click the "Create Tug of War Event" button above to publish your first team registration event for Tug of War.
            </p>
            <button
              onClick={handleOpenCreate}
              ...
              return (
                ...
                >
                  ...
                  <div className="relative h-44 w-full bg-slate-900 overflow-hidden">
                    <img
                      src={event.coverImage || 'https://images.unsplash.com/photo-1517649763962-0c623066013b?auto=format&fit=crop&w=800&q=80'}
                      alt={event.title}
      ...
      {/* CREATE / EDIT TUG OF WAR EVENT MODAL */}
      {showCreateModal && (
            ...
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
              <div>
                <span className="text-[10px] font-mono font-bold uppercase text-orange-600 dark:text-orange-400">
                  Tug of War Event Configurator
                </span>
                <h3 className="text-xl font-black text-slate-900 dark:text-white">
                  {editingEvent ? 'Edit Tug of War Event' : 'Create New Tug of War Event'}
                </h3>
              </div>
              <button
            ...
            <form onSubmit={handleSaveEvent} className="space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="sm:col-span-2 space-y-1.5">
                  ...
                  <input
                    ...
                    placeholder="e.g. Inter-College Tug of War Championship 2026"
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4">
                  ...
                  <div className="flex-1 space-y-2 w-full">
                    <p className="text-[11px] text-slate-600 dark:text-slate-400">
                      Upload a high-resolution Tug of War cover image. Click "Crop & Resize" to trim to standard 16:9 banner format before publishing.
                    </p>
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  placeholder="Enter detailed description of Tug of War tournament rules, round format, eligibility..."
              ...
              {/* TEAM FEE ONLY, MIN 8 & MAX 10 PLAYERS */}
              <div className="p-4 rounded-2xl bg-orange-500/5 border border-orange-500/20 space-y-4">
                <div className="flex items-center gap-2 border-b border-orange-500/20 pb-2">
                  ...
                  <h4 className="text-xs font-black uppercase text-orange-700 dark:text-orange-400 tracking-wider">
                    Tug of War Team Pricing & Squad Limits
                  </h4>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                  <div className="space-y-1 sm:col-span-2">
                    ...
                    />
                    <p className="text-[10px] text-slate-500 dark:text-slate-400">Flat registration fee collected per participating tug of war team.</p>
                  </div>
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  placeholder="e.g. Tug of War Ground 1"
              ...
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  placeholder="Official tournament rules apply&#10;Min 8 and Max 10 players required per squad&#10;Athletic footwear mandatory"
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-slate-200 dark:border-slate-800">
                ...
                >
                  {editingEvent ? 'Save Tug of War Event' : 'Publish Tug of War Event'}
                </button>
              </div>
                ...
                <button
                  onClick={() => {
                    const csvData = participants.map((p) => ({
                      ...
                      SquadCount: p.squadCount || '8-10',
                      Phone: p.contactPhone || p.phone,
                      Status: p.status,
                      RegisteredDate: p.registeredDate
                    }));
                    exportToCSV(csvData, `TugOfWar_Participants_${selectedEventForParticipants.id}`);
                    addToast('Exported Tug of War Roster as CSV/Excel', 'success');
                  }}
                  ...
                  {filteredParticipants.length === 0 ? (
                    ...
                    filteredParticipants.map((p) => (
                      <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-slate-900/50 transition">
                        ...
                        <td className="p-3 font-mono">8-10 Pullers</td>
