...
export const KhoKhoMatchScheduleTab = ({ matches, user, onUpdateMatches, globalSearch }) => {
    ...
    const fetchEvents = async () => {
      try {
        ...
        const filtered = (list || []).filter(
          (e) => (e.sportId || e.sportName || '').toLowerCase().includes('kho') || (e.title || '').toLowerCase().includes('kho')
        );
  ...
  useEffect(() => {
    ...
    const loadCompetitors = async () => {
      ...
      try {
        ...
        console.warn('Error loading eligible kho-kho teams:', e);
      } finally {
  ...
  const handleDeleteSlot = async (id) => {
    const isConfirmed = await confirmDelete({
      ...
      message: 'Are you sure you want to delete this scheduled Kho-Kho match fixture from the database?'
    });
      ...
      <div className="lg:col-span-4 space-y-4">
        ...
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-wider">
            {editingId ? 'Edit Kho-Kho Fixture' : 'Add Kho-Kho Fixture'}
          </h3>
        </div>
        ...
        {!selectedEvent ? (
          <div className="p-5 rounded-3xl bg-amber-500/10 border border-amber-500/30 text-amber-600 dark:text-amber-400 space-y-1.5">
            <h4 className="text-xs font-black uppercase tracking-wider">⚠️ No Active Kho-Kho Event</h4>
            <p className="text-xs text-slate-600 dark:text-slate-400">
              Create and publish a Kho-Kho event in the Events tab to enable match scheduling.
            </p>
          </div>
        ) : !isRegClosed ? (
          ...
          <form onSubmit={handleAddSlot} className="space-y-4">
            ...
            {isRegClosed && eligibleCompetitors.teams.length === 0 && (
              <p className="text-[11px] font-bold text-rose-500 dark:text-rose-400">
                ⚠️ No verified registrations found for this Kho-Kho event.
              </p>
            )}
