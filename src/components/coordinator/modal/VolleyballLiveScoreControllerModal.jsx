...
export const VolleyballLiveScoreControllerModal = ({ match, venueName, onClose, onMatchUpdated }) => {
  ...
  const handleFinishMatch = async () => {
    const defaultTeam1 = match?.team1 || 'Team 1';
    const defaultTeam2 = match?.team2 || 'Team 2';
    ...
    const matchId = match?.id || `M-VOL-${Math.floor(100000 + Math.random() * 900000)}`;
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999999] flex items-center justify-center p-3 sm:p-6 bg-slate-950/80 dark:bg-[#070B14]/90 backdrop-blur-md overflow-y-auto animate-fade-in font-sans">
