...
import { 
  X, RotateCcw, Trophy, AlertCircle, RefreshCw, UserCheck, Activity, Maximize2, Minimize2 
} from 'lucide-react';
import { useToast } from '../../../context/ToastContext';
import { coordinatorApi } from '../../../services/coordinatorApi';
import { generateMatchResultPDF } from '../../../utils/pdfExporter';

export const BasketballLiveScoreControllerModal = ({ match, venueName, onClose, onMatchUpdated }) => {
  ...
  // Default initial rosters if none provided
  const initialRoster1 = match?.roster1 && match.roster1.length >= 5
    ? match.roster1
    : [
        { id: 'T1-1', name: `${team1Name} Player 1`, jersey: '4', onCourt: true, points: 0, fouls: 0 },
        { id: 'T1-2', name: `${team1Name} Player 2`, jersey: '7', onCourt: true, points: 0, fouls: 0 },
        { id: 'T1-3', name: `${team1Name} Player 3`, jersey: '10', onCourt: true, points: 0, fouls: 0 },
        { id: 'T1-4', name: `${team1Name} Player 4`, jersey: '11', onCourt: true, points: 0, fouls: 0 },
        { id: 'T1-5', name: `${team1Name} Player 5`, jersey: '23', onCourt: true, points: 0, fouls: 0 },
  ...
  const initialRoster2 = match?.roster2 && match.roster2.length >= 5
    ? match.roster2
    : [
        { id: 'T2-1', name: `${team2Name} Player 1`, jersey: '4', onCourt: true, points: 0, fouls: 0 },
        { id: 'T2-2', name: `${team2Name} Player 2`, jersey: '7', onCourt: true, points: 0, fouls: 0 },
        { id: 'T2-3', name: `${team2Name} Player 3`, jersey: '10', onCourt: true, points: 0, fouls: 0 },
        { id: 'T2-4', name: `${team2Name} Player 4`, jersey: '11', onCourt: true, points: 0, fouls: 0 },
        { id: 'T2-5', name: `${team2Name} Player 5`, jersey: '23', onCourt: true, points: 0, fouls: 0 },
  ...
  // Calculate cumulative team total score from player points
  ...
  // Award Points to Player & Automatically update team total score
  const handleAwardPoints = (teamNum, playerId, pts) => {
    pushUndoState();
    ...
    const updated = list.map((p) => {
      if (p.id === playerId) {
        if (p.fouls >= 5) {
          addToast(`Player #${p.jersey} ${p.name} is Fouled Out and cannot score!`, 'error');
          return p;
        }
    ...
    if (player && pts > 0) {
      addToast(`🏀 +${pts} PTS awarded to #${player.jersey} ${player.name}`, 'success');
    }
  };

  // Award Personal Foul to Player (5 Fouls = Disqualified)
  const handleAwardFoul = (teamNum, playerId) => {
    pushUndoState();

    const list = teamNum === 1 ? roster1 : roster2;
    const setList = teamNum === 1 ? setRoster1 : setRoster2;

    let fouledOutPlayer = null;

    const updated = list.map((p) => {
      if (p.id === playerId) {
        const nextFouls = (p.fouls || 0) + 1;
        if (nextFouls >= 5) {
          fouledOutPlayer = p;
        }
        return { ...p, fouls: nextFouls };
      }
      return p;
    });
    ...
    const player = list.find((p) => p.id === playerId);
    if (fouledOutPlayer) {
      addToast(`⚠️ FOULED OUT! #${player.jersey} ${player.name} reached 5 fouls (Disqualified & Unavailable to play)`, 'error');
    } else if (player) {
      addToast(`🚨 Foul #${(player.fouls || 0) + 1} given to #${player.jersey} ${player.name}`, 'warning');
    }
  };

  // Perform Substitution (Swap On-Court player with Bench player)
  const handleExecuteSub = (teamNum, outPlayerId, inPlayerId) => {
    ...
    if (inPlayer && inPlayer.fouls >= 5) {
      addToast(`Cannot sub in #${inPlayer.jersey} ${inPlayer.name} (Fouled Out / Disqualified)`, 'error');
      return;
    }
  ...
  // Next Quarter transition (Scores DO NOT reset!)
  const handleNextQuarter = () => {
    const quarterOrder = ['Quarter 1', 'Quarter 2', 'Quarter 3', 'Quarter 4', 'Overtime (OT)'];
    ...
    if (currentIdx < quarterOrder.length - 1) {
      ...
      addToast(`🏀 Advanced to ${nextQ}. Team total points continue!`, 'success');
    } else {
      addToast('Match is in Overtime (OT)', 'info');
    }
  };
  ...
  const handleFinishMatch = async () => {
    ...
    if (!window.confirm(`Finish Basketball match with score ${team1Name} ${s1} - ${s2} ${team2Name}?`)) return;
    ...
    try {
      ...
      addToast(`🏆 Basketball Match Completed! Winner: ${winnerName}. PDF downloaded.`, 'success');
      onClose();
    } catch (err) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999999] bg-slate-100 dark:bg-[#060911] text-slate-900 dark:text-white flex flex-col font-sans overflow-hidden w-screen h-screen select-none p-2 sm:p-3 space-y-2 transition-colors">
      ...
      <header className="px-4 py-2.5 bg-white dark:bg-[#0D1424] border border-slate-200 dark:border-slate-800 rounded-2xl flex flex-wrap items-center justify-between gap-3 shrink-0 shadow-lg transition-colors">
        ...
        {/* Center: Quarter Indicator & Fast Tools */}
        <div className="flex items-center gap-2">
        ...
        {/* Right: Team 2 Score & Exit Actions */}
        <div className="flex items-center gap-3">
      ...
      {/* 2. DENSE COMPACT 2-COLUMN ARENA PLAYING GRID (FITS 100% ON SCREEN) */}
                ...
                >
                  ...
                  {isFouledOut ? (
                    ...
                    <div className="grid grid-cols-4 gap-1">
                      <button
                        type="button"
                        onClick={() => handleAwardPoints(1, player.id, 3)}
                        className="py-1 rounded-lg bg-orange-600 hover:bg-orange-500 text-white font-black text-xs transition cursor-pointer active:scale-95 shadow-xs"
                      >
                        +3 PTS
                      </button>
                      <button
                        type="button"
                        onClick={() => handleAwardPoints(1, player.id, 2)}
                        className="py-1 rounded-lg bg-amber-600 hover:bg-amber-500 text-white font-black text-xs transition cursor-pointer active:scale-95 shadow-xs"
                      >
                        +2 PTS
                      </button>
                      <button
                ...
                >
                  ...
                  {isFouledOut ? (
                    ...
                    <div className="grid grid-cols-4 gap-1">
                      <button
                        type="button"
                        onClick={() => handleAwardPoints(2, player.id, 3)}
                        className="py-1 rounded-lg bg-blue-600 hover:bg-blue-500 text-white font-black text-xs transition cursor-pointer active:scale-95 shadow-xs"
                      >
                        +3 PTS
                      </button>
                      <button
                        type="button"
                        onClick={() => handleAwardPoints(2, player.id, 2)}
                        className="py-1 rounded-lg bg-cyan-600 hover:bg-cyan-500 text-white font-black text-xs transition cursor-pointer active:scale-95 shadow-xs"
                      >
                        +2 PTS
                      </button>
                      <button
                ...
                <button
                  ...
                  className={`px-2 py-0.5 rounded text-[10px] font-bold border shrink-0 transition flex items-center gap-1 cursor-pointer ${
                    (bp.fouls || 0) >= 5
                      ...
                      : 'bg-[#121A2D] hover:bg-blue-500/20 text-slate-300 border-slate-700'
                  }`}
                >
          ...
          <div className="w-full max-w-md bg-[#0D1424] text-white rounded-3xl border border-slate-800 p-6 shadow-2xl space-y-4">
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto custom-scrollbar">
              {(subModal.teamNum === 1 ? roster1 : roster2)
                .filter((p) => !p.onCourt)
                .map((benchPlayer) => {
                  const isFouledOut = (benchPlayer.fouls || 0) >= 5;
                    ...
                    <button
                      ...
                      className={`w-full p-3 rounded-2xl border text-left flex items-center justify-between transition ${
                        isFouledOut
                          ...
                          : 'bg-[#121A2D] border-slate-800 hover:border-orange-500 cursor-pointer'
                      }`}
                    >
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          ...
                          <div className="text-[10px] text-slate-400 font-mono">
                            {benchPlayer.points || 0} pts | {benchPlayer.fouls || 0}/5 fouls
                          </div>
                        </div>
                      </div>

                      {isFouledOut ? (
                        <span className="text-[10px] font-bold text-rose-500">Fouled Out</span>
                      ) : (
