...
export const BasketballRosterSetupModal = ({ match, targetVenue, onClose, onRosterSaved }) => {
  ...
  const [team1Name, setTeam1Name] = useState(match?.team1 || 'ARC');
  const [team2Name, setTeam2Name] = useState(match?.team2 || 'RCD');

  // Default seed roster generator (min 5, max 10 players)
  const defaultRosterForTeam = (teamName, prefix) => [
    { id: `${prefix}-1`, name: `${teamName} Player 1`, jersey: '4', onCourt: true, points: 0, fouls: 0 },
    { id: `${prefix}-2`, name: `${teamName} Player 2`, jersey: '7', onCourt: true, points: 0, fouls: 0 },
    { id: `${prefix}-3`, name: `${teamName} Player 3`, jersey: '10', onCourt: true, points: 0, fouls: 0 },
    { id: `${prefix}-4`, name: `${teamName} Player 4`, jersey: '11', onCourt: true, points: 0, fouls: 0 },
    { id: `${prefix}-5`, name: `${teamName} Player 5`, jersey: '23', onCourt: true, points: 0, fouls: 0 },
    { id: `${prefix}-6`, name: `${teamName} Sub 1`, jersey: '30', onCourt: false, points: 0, fouls: 0 },
    { id: `${prefix}-7`, name: `${teamName} Sub 2`, jersey: '33', onCourt: false, points: 0, fouls: 0 },
  ];
  ...
  useEffect(() => {
    ...
    const fetchRegisteredRosters = async () => {
      try {
        ...
        console.warn('Could not auto-populate registered basketball rosters:', err);
      }
    };
  ...
  const handleToggleOnCourt = (teamNum, id) => {
    ...
    if (!targetPlayer.onCourt && currentOnCourtCount >= 5) {
      addToast(`Exactly 5 players can be on court for ${currentTeam}`, 'warning');
      return;
    }
  ...
  const handleSaveRoster = (e) => {
    ...
    if (roster1.length < 5 || roster1.length > 10) {
      addToast(`Team 1 (${team1Name}) must have between 5 and 10 players`, 'error');
      return;
    }

    if (roster2.length < 5 || roster2.length > 10) {
      addToast(`Team 2 (${team2Name}) must have between 5 and 10 players`, 'error');
      return;
    }
    ...
    if (onCourt1 !== 5) {
      addToast(`Exactly 5 players must be selected for ${team1Name}'s Starting 5 (Currently selected: ${onCourt1})`, 'error');
      return;
    }

    if (onCourt2 !== 5) {
      addToast(`Exactly 5 players must be selected for ${team2Name}'s Starting 5 (Currently selected: ${onCourt2})`, 'error');
      return;
    }
        ...
        <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
          <div>
            ...
            <h3 className="text-xl font-black text-slate-900 dark:text-white">
              Basketball Squad Roster Setup & Starting 5
            </h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
              Customize Team Names, player names & jersey numbers (5-10 per squad) and select the Starting 5 on court for {targetVenue}.
            </p>
          </div>
          <button
            onClick={onClose}
              ...
              <div className="flex items-center justify-between border-b border-orange-500/20 pb-2 gap-2">
                <div className="flex-1">
                  ...
                  <input
                    ...
                    placeholder="Team 1 Name (e.g. ARC)"
                ...
                <div className="flex items-center gap-2 shrink-0 pt-4">
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-orange-500/20 text-orange-600 dark:text-orange-400">
                    Starting 5: {roster1.filter((p) => p.onCourt).length}/5
                  </span>
                  <button
                    type="button"
                ...
                {roster1.map((player, idx) => (
                  ...
                  >
                    <button
                      ...
                      title={player.onCourt ? 'On Court (Starting 5)' : 'Bench / Substitute'}
                    >
                    ...
                    <input
                      ...
                      placeholder={`Player Name (${team1Name})`}
              ...
              <div className="flex items-center justify-between border-b border-blue-500/20 pb-2 gap-2">
                <div className="flex-1">
                  ...
                  <input
                    ...
                    placeholder="Team 2 Name (e.g. RCD)"
                ...
                <div className="flex items-center gap-2 shrink-0 pt-4">
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-blue-500/20 text-blue-600 dark:text-blue-400">
                    Starting 5: {roster2.filter((p) => p.onCourt).length}/5
                  </span>
                  <button
                    type="button"
                ...
                {roster2.map((player, idx) => (
                  ...
                  >
                    ...
                    <input
                      ...
                      placeholder={`Player Name (${team2Name})`}
