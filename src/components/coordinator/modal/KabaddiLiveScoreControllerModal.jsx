...
export const KabaddiLiveScoreControllerModal = ({ match, venueName, onClose, onMatchUpdated }) => {
  ...
  return createPortal(
    ...
    <div className="fixed inset-0 z-[999999] bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-3 sm:p-5 overflow-y-auto animate-fade-in font-sans">
      
      {/* CONTAINER CARD */}
      <div className="w-full max-w-4xl bg-[#060A14] border border-slate-800 rounded-3xl shadow-2xl overflow-hidden text-white my-auto p-4 sm:p-5 space-y-3.5">
        
        {/* TOP NAV CONTROL BAR */}
        <div className="flex items-center justify-between bg-[#0B1120] px-4 py-2.5 rounded-xl border border-slate-800 text-xs">
          <div className="flex items-center gap-2">
            <span className="w-2.5 h-2.5 rounded-full bg-rose-500 animate-pulse" />
            <span className="text-xs font-mono font-bold text-blue-400 uppercase tracking-wider">{venueName || 'Kabaddi Mat Arena 1'}</span>
            <span className="hidden sm:inline-block text-slate-500 font-mono text-xs">| Official Match Console</span>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-amber-400 font-bold text-xs border border-slate-700 transition flex items-center gap-1 cursor-pointer"
        ...
        {completedHalf1 && (
          <div className="bg-gradient-to-r from-amber-500/10 via-purple-500/20 to-blue-500/10 border border-purple-500/30 rounded-2xl p-3 text-center space-y-1">
            <span className="text-[10px] font-mono font-black text-amber-400 uppercase tracking-widest flex items-center justify-center gap-1.5">
              <Award className="w-3.5 h-3.5 text-amber-400" /> 1ST HALF COMPLETED RESULT SUMMARY
            </span>
            <div className="text-sm sm:text-base font-mono font-black text-white flex items-center justify-center gap-3">
              <span className="text-blue-400">{team1Name}: {half1Score1} Pts</span>
        ...
        <div className="bg-[#0A101D] border border-slate-800/80 rounded-2xl p-4 sm:p-5 shadow-xl space-y-3">
          ...
          <div className="grid grid-cols-12 items-center gap-2">
            ...
            <div className="col-span-4 text-center space-y-1.5">
              ...
              <div className="flex items-center justify-center gap-3 sm:gap-5 text-4xl sm:text-5xl font-black font-mono leading-none">
                <span className="text-blue-400 drop-shadow-md">{activeScore1}</span>
                <span className="text-slate-600 font-normal text-3xl sm:text-4xl">|</span>
                <span className="text-blue-400 drop-shadow-md">{activeScore2}</span>
              </div>
              ...
              <div>
                {half === 1 ? (
                  ...
                  <span className="px-4 py-1 rounded-full bg-emerald-600/20 text-emerald-400 border border-emerald-500/30 font-mono text-xs font-black tracking-wider uppercase inline-block">
          ...
          <div className="grid grid-cols-5 items-center justify-between bg-[#080D18] p-2.5 rounded-xl border border-slate-800/80 text-center font-mono pt-2">
            <div>
              <div className="text-sm sm:text-xl font-black text-white">{activeStats1.raid || 0}</div>
              <div className="text-[8px] sm:text-[9px] font-bold text-slate-400 uppercase tracking-wider">RAID POINTS</div>
            </div>

            <div>
              <div className="text-sm sm:text-xl font-black text-white">{activeStats1.tackle || 0}</div>
              <div className="text-[8px] sm:text-[9px] font-bold text-slate-400 uppercase tracking-wider">TACKLE POINTS</div>
            </div>

            {/* Center BONUS Indicator Dots */}
            <div className="space-y-0.5">
              <div className="text-[9px] font-mono font-black text-slate-400 uppercase tracking-widest">BONUS</div>
              <div className="flex items-center justify-center gap-1">
                <span className={`w-3 h-3 rounded-full border ${activeStats1.bonus > 0 || activeStats2.bonus > 0 ? 'bg-blue-500 border-blue-400' : 'bg-slate-800 border-blue-500/40'}`} />
                <span className={`w-3 h-3 rounded-full border ${activeStats1.bonus > 1 || activeStats2.bonus > 1 ? 'bg-blue-500 border-blue-400' : 'bg-slate-800 border-blue-500/40'}`} />
                <span className={`w-3 h-3 rounded-full border ${activeStats1.bonus > 2 || activeStats2.bonus > 2 ? 'bg-blue-500 border-blue-400' : 'bg-slate-800 border-blue-500/40'}`} />
                <span className={`w-3 h-3 rounded-full border ${activeStats1.bonus > 3 || activeStats2.bonus > 3 ? 'bg-blue-500 border-blue-400' : 'bg-slate-800 border-blue-500/40'}`} />
              </div>
            </div>

            <div>
              <div className="text-sm sm:text-xl font-black text-white">{activeStats2.tackle || 0}</div>
              <div className="text-[8px] sm:text-[9px] font-bold text-slate-400 uppercase tracking-wider">TACKLE POINTS</div>
            </div>

            <div>
              <div className="text-sm sm:text-xl font-black text-white">{activeStats2.raid || 0}</div>
              <div className="text-[8px] sm:text-[9px] font-bold text-slate-400 uppercase tracking-wider">RAID POINTS</div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
          ...
          <div className="bg-[#0A101D] border border-slate-800/80 rounded-2xl p-4 sm:p-5 shadow-xl space-y-3">
            ...
            <div className="space-y-2 border-b border-slate-800/80 pb-2.5">
              <span className="text-xs font-mono font-black text-blue-400 uppercase tracking-wider block">
                TEAM A ACTIONS ({team1Name}) - {half === 1 ? 'HALF 1' : 'HALF 2'}
              </span>

              {/* Live Category Stats Row Directly Inside Card */}
              <div className="grid grid-cols-5 gap-1 text-center text-[9px] font-mono font-bold bg-[#111929] p-1.5 rounded-lg border border-slate-800">
                <div>
                  <span className="text-slate-400 block text-[8px]">RAID</span>
                  <span className="text-blue-400 font-black text-xs">{activeStats1.raid || 0}</span>
                </div>
                <div>
                  <span className="text-slate-400 block text-[8px]">TACKLE</span>
                  <span className="text-blue-400 font-black text-xs">{activeStats1.tackle || 0}</span>
                </div>
                <div>
                  <span className="text-slate-400 block text-[8px]">BONUS</span>
                  <span className="text-blue-400 font-black text-xs">{activeStats1.bonus || 0}</span>
                </div>
                <div>
                  <span className="text-slate-400 block text-[8px]">S.TACKLE</span>
                  <span className="text-blue-400 font-black text-xs">{activeStats1.superTackle || 0}</span>
                </div>
                <div>
                  <span className="text-slate-400 block text-[8px]">S.RAID</span>
                  <span className="text-blue-400 font-black text-xs">{activeStats1.superRaid || 0}</span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2.5">
              ...
              <button
                ...
                className="py-2.5 px-3 rounded-xl bg-[#121A2B] hover:bg-[#1C273D] text-white font-mono font-black text-xs sm:text-sm border border-blue-500/40 shadow-xs transition active:scale-95 cursor-pointer"
              >
                + 1 TACKLE
              </button>
              ...
              <button
                ...
                className="py-2.5 px-3 rounded-xl bg-[#121A2B] hover:bg-slate-800 text-slate-300 font-mono font-bold text-xs sm:text-sm border border-slate-700 transition active:scale-95 flex items-center justify-center gap-1.5 cursor-pointer"
              >
                <RotateCcw className="w-3.5 h-3.5 text-blue-400" /> UNDO
              </button>
            </div>

            {/* Team A Separate Total Summary Bar */}
            <div className="pt-2 border-t border-slate-800/80 grid grid-cols-3 gap-2 font-mono text-center">
              <div className="bg-blue-600/10 p-1.5 rounded-lg border border-blue-500/20">
                <span className="text-[8px] text-slate-400 block uppercase font-bold">{half === 1 ? 'H1 RAIDS' : 'H2 RAIDS'}</span>
                <span className="font-black text-xs text-white">{activeTeam1TotalRaids}</span>
              </div>
              <div className="bg-blue-600/10 p-1.5 rounded-lg border border-blue-500/20">
                <span className="text-[8px] text-slate-400 block uppercase font-bold">{half === 1 ? 'H1 TACKLES' : 'H2 TACKLES'}</span>
                <span className="font-black text-xs text-white">{activeTeam1TotalTackles}</span>
              </div>
              <div className="bg-blue-600/10 p-1.5 rounded-lg border border-blue-500/20">
                <span className="text-[8px] text-slate-400 block uppercase font-bold">{half === 1 ? 'H1 BONUS' : 'H2 BONUS'}</span>
                <span className="font-black text-xs text-white">{activeTeam1TotalBonus}</span>
              </div>
            </div>
          ...
          <div className="bg-[#0A101D] border border-slate-800/80 rounded-2xl p-4 sm:p-5 shadow-xl space-y-3">
            ...
            <div className="space-y-2 border-b border-slate-800/80 pb-2.5">
              ...
              <div className="grid grid-cols-5 gap-1 text-center text-[9px] font-mono font-bold bg-[#111929] p-1.5 rounded-lg border border-slate-800">
                <div>
                  <span className="text-slate-400 block text-[8px]">RAID</span>
                  <span className="text-blue-400 font-black text-xs">{activeStats2.raid || 0}</span>
                </div>
                <div>
                  <span className="text-slate-400 block text-[8px]">TACKLE</span>
                  <span className="text-blue-400 font-black text-xs">{activeStats2.tackle || 0}</span>
                </div>
                <div>
                  <span className="text-slate-400 block text-[8px]">BONUS</span>
                  <span className="text-blue-400 font-black text-xs">{activeStats2.bonus || 0}</span>
                </div>
                <div>
                  <span className="text-slate-400 block text-[8px]">S.TACKLE</span>
                  <span className="text-blue-400 font-black text-xs">{activeStats2.superTackle || 0}</span>
                </div>
                <div>
                  <span className="text-slate-400 block text-[8px]">S.RAID</span>
                  <span className="text-blue-400 font-black text-xs">{activeStats2.superRaid || 0}</span>
                </div>
              </div>
            </div>
            ...
            <div className="pt-2 border-t border-slate-800/80 grid grid-cols-3 gap-2 font-mono text-center">
              <div className="bg-blue-600/10 p-1.5 rounded-lg border border-blue-500/20">
                <span className="text-[8px] text-slate-400 block uppercase font-bold">{half === 1 ? 'H1 RAIDS' : 'H2 RAIDS'}</span>
                <span className="font-black text-xs text-white">{activeTeam2TotalRaids}</span>
              </div>
              <div className="bg-blue-600/10 p-1.5 rounded-lg border border-blue-500/20">
                <span className="text-[8px] text-slate-400 block uppercase font-bold">{half === 1 ? 'H1 TACKLES' : 'H2 TACKLES'}</span>
                <span className="font-black text-xs text-white">{activeTeam2TotalTackles}</span>
              </div>
              <div className="bg-blue-600/10 p-1.5 rounded-lg border border-blue-500/20">
                <span className="text-[8px] text-slate-400 block uppercase font-bold">{half === 1 ? 'H1 BONUS' : 'H2 BONUS'}</span>
                <span className="font-black text-xs text-white">{activeTeam2TotalBonus}</span>
              </div>
            </div>
