...
export const CoordinatorSidebar = ({ activeTab, setActiveTab, user, onLogout, isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      <aside className={`
        fixed lg:sticky top-0 left-0 z-50 h-screen w-72 bg-[#111827] border-r border-slate-800 
        flex flex-col justify-between transition-transform duration-300 ease-in-out shrink-0 text-white
      ...
      `}>
        ...
        <div className="flex-1 overflow-y-auto px-3 py-6 space-y-2">
          <span className="text-[10px] font-black uppercase text-slate-500 tracking-widest px-3 block mb-3">
              ...
              >
                ...
                {tab.badge && (
                  <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase ${
                    isActive ? 'bg-white text-indigo-700' : 'bg-rose-500/20 text-rose-400 border border-rose-500/30 animate-pulse'
                  }`}>
                    {tab.badge}
                  </span>
                )}
        ...
        <div className="p-4 border-t border-slate-800 bg-[#0B1120]">
          <div className="flex items-center justify-between">
            <div className="truncate">
              <p className="text-xs font-black text-white truncate">{user?.coordinatorName || 'Table Tennis Official'}</p>
              <p className="text-[10px] text-slate-400 truncate">{user?.email || 'coord@sems.edu'}</p>
            </div>
            <button
              onClick={onLogout}
