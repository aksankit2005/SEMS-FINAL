      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-xl w-full p-6 shadow-2xl relative space-y-5 max-h-[90vh] overflow-y-auto custom-scrollbar">
        ...
        <div className="flex items-center justify-between border-b border-slate-800 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Update official First (🥇) & Second (🥈) positions</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-semibold text-slate-300 block mb-1">Sport / Game</label>
              <select
                value={formData.sportId}
                onChange={(e) => handleSportChange(e.target.value)}
                className="w-full bg-slate-800/70 border border-slate-700 rounded-xl px-3 py-2.5 text-xs text-white focus:outline-none focus:border-amber-500"
              >
                {ALL_12_SPORTS.map(s => (
            ...
            <div>
              <label className="text-xs font-semibold text-slate-300 block mb-1">Event / Tournament Title</label>
              <input
                type="text"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="text-xs font-semibold text-slate-300 block mb-1">Gender Category</label>
              <select
          ...
          {/* Score & Points */}
          <div>
            <label className="text-xs font-semibold text-slate-300 block mb-1">Score / Final Result Details</label>
            <input
              type="text"
