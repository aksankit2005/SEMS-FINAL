...
export const RegistrationDetailsModal = ({ isOpen, registration, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm animate-fade-in">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-2xl w-full p-6 shadow-2xl relative space-y-6 max-h-[90vh] overflow-y-auto custom-scrollbar">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-slate-800 pb-4">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-slate-400">ID: {registration.id || registration.registrationId}</span>
            </div>
            <h3 className="text-xl font-black text-white mt-1">{registration.participantName}</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="p-3.5 rounded-xl bg-slate-800/40 border border-slate-800 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs font-bold text-white">{registration.rollNumber || 'N/A'}</p>
            </div>
          </div>
          ...
          <div className="p-3.5 rounded-xl bg-slate-800/40 border border-slate-800 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs font-bold text-white">{registration.college || 'MPEC Kanpur'}</p>
            </div>
          </div>
          ...
          <div className="p-3.5 rounded-xl bg-slate-800/40 border border-slate-800 flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] text-slate-400 font-semibold uppercase">Branch & Year</p>
          ...
          <div className="p-3.5 rounded-xl bg-slate-800/40 border border-slate-800 flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] text-slate-400 font-semibold uppercase">Game & Category</p>
              <p className="text-xs font-bold text-white">
          ...
          <div className="p-3.5 rounded-xl bg-slate-800/40 border border-slate-800 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs font-bold text-white">{registration.mobile || 'N/A'}</p>
            </div>
          </div>
          ...
          <div className="p-3.5 rounded-xl bg-slate-800/40 border border-slate-800 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs font-bold text-white truncate">{registration.email || 'N/A'}</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl bg-slate-950 border border-slate-800 space-y-3">
          <h4 className="text-xs font-bold text-slate-300 uppercase tracking-wider">Registration Metadata</h4>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
            <div>
              <span className="text-slate-500 text-[10px] block">Reg. Date</span>
              <span className="font-semibold text-slate-200">{registration.registrationDate}</span>
            </div>
            <div>
              <span className="text-slate-500 text-[10px] block">Reg. Time</span>
              <span className="font-semibold text-slate-200">{registration.registrationTime || '10:00 AM'}</span>
            </div>
            <div>
              <span className="text-slate-500 text-[10px] block">Payment Status</span>
              <span className="inline-block px-2 py-0.5 text-[10px] font-bold rounded bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 mt-0.5">
                {registration.paymentStatus || 'PAID'}
              </span>
            </div>
            <div>
              <span className="text-slate-500 text-[10px] block">Registered By</span>
              <span className="font-semibold text-slate-200 truncate block">{registration.registeredBy}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-end pt-2">
          <button
            onClick={onClose}
            className="px-5 py-2.5 rounded-xl text-xs font-bold bg-slate-800 hover:bg-slate-700 text-white border border-slate-700 transition-colors"
          >
            Close Window
          </button>
        </div>
