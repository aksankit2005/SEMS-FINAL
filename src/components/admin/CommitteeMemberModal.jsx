...
export const CommitteeMemberModal = ({ isOpen, member = null, onSave, onClose }) => {
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      const optimized = await compressImageBeforeUpload(file, 800, 0.85);
        ...
        <div className="flex items-center justify-between border-b border-slate-800 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Set photo, name & position</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center gap-4 p-4 rounded-xl bg-slate-800/40 border border-slate-700">
            ...
            <div className="flex-1 min-w-0 space-y-2">
              <label className="block text-[11px] font-bold text-slate-400 uppercase">Member Photo</label>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {formData.image && (
                  <button
                    type="button"
                    onClick={() => setFormData({ ...formData, image: FALLBACK_IMAGE })}
              ...
              <p className="text-[10px] text-slate-500">Photo is auto-compressed. Leave empty to use a default avatar.</p>
            </div>
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-slate-800/70 border border-slate-700 rounded-xl px-3 py-2.5 text-xs text-white focus:outline-none focus:border-indigo-500"
            />
          </div>

          {/* Position / Role */}
          <div>
            <label className="text-xs font-semibold text-slate-300 block mb-1">Position / Role *</label>
            <input
              type="text"
              value={formData.role}
